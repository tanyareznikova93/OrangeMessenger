package com.example.orangemessenger.ui.screens.single_chat
...
class SingleChatFragment(private val contact: CommonModel) : BaseFragment(R.layout.fragment_single_chat) {
    ...
    private fun initFields() {
        ...
        chat_input_message_et.addTextChangedListener(AppTextWatcher{
            val string = chat_input_message_et.text.toString()
            if(string.isEmpty() || string == "Запись"){
                chat_btn_send_message_iv.visibility = View.GONE
                chat_btn_attach_iv.visibility = View.VISIBLE
                chat_btn_voice_iv.visibility = View.VISIBLE
            } else {
                chat_btn_send_message_iv.visibility = View.VISIBLE
                chat_btn_attach_iv.visibility = View.GONE
                chat_btn_voice_iv.visibility = View.GONE
            }
        })

        chat_btn_attach_iv.setOnClickListener { attach() }

        CoroutineScope(Dispatchers.IO).launch {

            chat_btn_voice_iv.setOnTouchListener { v, event ->
                if(checkPermission(RECORD_AUDIO)){
                    if(event.action == MotionEvent.ACTION_DOWN){
                        //TODO record
                        chat_input_message_et.setText("Запись")
                        chat_btn_voice_iv.setColorFilter(ContextCompat.getColor(APP_ACTIVITY,R.color.md_blue_400))
                        val messageKey = getMessageKey(contact.id)
                        mAppVoiceRecorder.startRecord(messageKey)
                    } else if(event.action == MotionEvent.ACTION_UP) {
                        //TODO stop record
                        chat_input_message_et.setText("")
                        chat_btn_voice_iv.setColorFilter(null)
    ...
    private fun initToolbar() {
        ...
        chat_btn_send_message_iv.setOnClickListener {
            mSmoothScrollToPosition = true
    ...
    private fun initInfoToolbar() {
        if(mReceivingUser.fullname.isEmpty()){
            mToolbarInfo.toolbar_chat_fullname_tv.text = contact.fullname
